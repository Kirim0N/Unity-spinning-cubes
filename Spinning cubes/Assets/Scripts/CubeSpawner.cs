...
public class CubeSpawner : MonoBehaviour
{
    ...
    private void Awake()
    {
        for (var i = 0; i < cubesNumber; i++)
        {
            ...
            Instantiate(cubePrefab, new Vector3((float) Mathf.Cos((2 * Mathf.PI / cubesNumber) * i) * radius, 0, (float) Mathf.Sin((2 * Mathf.PI / cubesNumber) * i) * radius), Quaternion.Euler(0, 0, 0), cubeParent);
        }
    }
